\documentclass[11pt]{article}
    ...
    % Document parameters
    % Document title
    \title{datacell\_request\_time\_v2}
...
\begin{document}
    ...
    A markdown cells in \textbf{bold text and default size} indicate that
you should execute the cell below it, ex.:
...
of each new kernel session, execution cells must be reloaded every time
...
indicated by the above notation.
    ...
    Eugene: you can experiemnt with turning matplotlib inline on and off, it
doesn't seem to have an effect as I think Seaborn is automatically
running show() on each of the plots. But truning it on won't hurt.

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{58}{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
\PY{c+c1}{\PYZsh{} Turn on inline display of plots in the notebook. (We should work on a more}
\PY{c+c1}{\PYZsh{} portable solution such as saving th plots to files down the road.)}
...
\end{tcolorbox}
    ...
    Eugene: Robby helped me think of a way to possibly improve performance
accross the entire notebook by first importing the data from the SQL DB
into "pure python" (instead of directly into a pandas dataframe) using
pyodbc and putting the data first in a numpy matrix (or possibly JSON
obj), then using the numpy matrix to create pandas dataframes later as
they are needed. This would help because Numpy is written in C and it
highly optimized for large amounts of data.
...
functions:}\label{dataframe-generator-functions}
    ...
    The following functions generate new DFs either based on filtering
another DF being passed in, or create a new DF using
get\_df\_requests\_with\_date\_cols\_from\_db() and then filtering the
...
\begin{Verbatim}[commandchars=\\\{\}]
\PY{c+c1}{\PYZsh{} Make labels readable. Exclude weekends. Exclude holidays? Make if very obvious that we are excluding thesedays, make this reversible.}
\end{Verbatim}
\end{tcolorbox}
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{datacell_request_time_v2_files/datacell_request_time_v2_62_0.png}
    \end{center}
    { \hspace*{\fill} \\}
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{datacell_request_time_v2_files/datacell_request_time_v2_70_0.png}
    \end{center}
    { \hspace*{\fill} \\}
    
    As opposed to \textbf{this}:

    The following was the "old way" that I tried to solve the problem of
making a separate subplot for each month of data, where I had broken it
up into 12 dataframes. This is the wrong solution because it does a lot
of unecessary data manipulation in pandas. I then discovered Seaborn's
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{datacell_request_time_v2_files/datacell_request_time_v2_86_0.png}
    \end{center}
    { \hspace*{\fill} \\}
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{datacell_request_time_v2_files/datacell_request_time_v2_90_0.png}
    \end{center}
    { \hspace*{\fill} \\}
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{datacell_request_time_v2_files/datacell_request_time_v2_98_0.png}
    \end{center}
    { \hspace*{\fill} \\}
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{datacell_request_time_v2_files/datacell_request_time_v2_113_0.png}
    \end{center}
    { \hspace*{\fill} \\}
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{datacell_request_time_v2_files/datacell_request_time_v2_117_0.png}
    \end{center}
    { \hspace*{\fill} \\}
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{datacell_request_time_v2_files/datacell_request_time_v2_128_0.png}
    \end{center}
    { \hspace*{\fill} \\}
...
months}\label{daily-stats-over-3-months}
    ...
    Fine-tuning done... TO DO: -Apply month-naming function and add Axes
titles.
...
\end{tcolorbox}
    ...
    Was writing this to debug the problem with there being gaps in the mean
estimator range on the lineplot for plot 3.3.1:
...
plot}\label{daily-stats-over-3-months-in-single-plot}
    ...
    Eugene: This graph was producing some strange output where it would show
a standard deviation from the mean but only for discrete ranges of
x-values, then the std. dev graphic would not show up, then show again
depending on the x-value. I figured out that because I was plotting the
'Day' column on the x-axis, it was plotting each unique DAY value,
irrespective of what month/year that day was in. So instead, I reindexed
the DataFrame so that there was a unique integer for each observation,
then used this index as the x-value for the plot. This also fixed the
problem I had with the Day\_Label not matching the the day, for obvious
reasons. The graph was just bucketing every observation on day "3", for
example, into one x-point, which is why we had standard devs. only
sometimes.
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{datacell_request_time_v2_files/datacell_request_time_v2_142_0.png}
    \end{center}
    { \hspace*{\fill} \\}
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{datacell_request_time_v2_files/datacell_request_time_v2_146_0.png}
    \end{center}
    { \hspace*{\fill} \\}
    
    \paragraph{For swarmplot (buggy)}\label{for-swarmplot-buggy}

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
\PY{n}{longform\PYZus{}dur\PYZus{}stats\PYZus{}3\PYZus{}mon\PYZus{}by\PYZus{}day} \PY{o}{=} \PY{n}{pd}\PY{o}{.}\PY{n}{melt}\PY{p}{(}\PY{n}{dur\PYZus{}stats\PYZus{}3\PYZus{}mon\PYZus{}by\PYZus{}day}\PY{p}{,} \PY{n}{id\PYZus{}vars}\PY{o}{=}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Day}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{n}{value\PYZus{}vars}\PY{o}{=}\PY{p}{[}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Min\PYZus{}Duration}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Max\PYZus{}Duration}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Mean\PYZus{}Duration}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Count\PYZus{}Durations}\PY{l+s+s1}{\PYZsq{}}\PY{p}{]}\PY{p}{,} \PY{n}{var\PYZus{}name}\PY{o}{=}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Dur\PYZus{}Stat\PYZus{}Type}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{n}{value\PYZus{}name}\PY{o}{=}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Dur\PYZus{}Stat\PYZus{}Value}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}
\PY{c+c1}{\PYZsh{} longform\PYZus{}dur\PYZus{}stats\PYZus{}3\PYZus{}mon\PYZus{}by\PYZus{}day.set\PYZus{}index(keys=\PYZsq{}Day\PYZsq{}, inplace=True) \PYZsh{} duplicate keys OK!}

\PY{c+c1}{\PYZsh{} Get Date and Weekday from hist\PYZus{}3\PYZus{}mon:}
\PY{c+c1}{\PYZsh{} Is there a better way to get the time stamp?}
\PY{c+c1}{\PYZsh{} hist\PYZus{}3\PYZus{}mon\PYZus{}day\PYZus{}index = history\PYZus{}3\PYZus{}mon.set\PYZus{}index(keys=\PYZsq{}Day\PYZsq{})}
\PY{c+c1}{\PYZsh{} history\PYZus{}3\PYZus{}mon}
\PY{c+c1}{\PYZsh{} hist\PYZus{}3\PYZus{}mon\PYZus{}day\PYZus{}index = hist\PYZus{}3\PYZus{}mon\PYZus{}day\PYZus{}index.groupby(\PYZsq{}Day\PYZsq{}).max()}
\PY{c+c1}{\PYZsh{} hist\PYZus{}3\PYZus{}mon\PYZus{}day\PYZus{}index[\PYZsq{}StartTimeStamp\PYZsq{}] = hist\PYZus{}3\PYZus{}mon\PYZus{}day\PYZus{}index[\PYZsq{}StartTimeStamp\PYZsq{}].astype(str).str.split(\PYZsq{} \PYZsq{}, expand=True)[0]}
\PY{c+c1}{\PYZsh{} hist\PYZus{}3\PYZus{}mon\PYZus{}day\PYZus{}index = hist\PYZus{}3\PYZus{}mon\PYZus{}day\PYZus{}index[[\PYZsq{}StartTimeStamp\PYZsq{}, \PYZsq{}Weekday\PYZsq{}, \PYZsq{}Month\PYZsq{}]]}
\PY{c+c1}{\PYZsh{} hist\PYZus{}3\PYZus{}mon\PYZus{}day\PYZus{}index.head()}
\PY{c+c1}{\PYZsh{} \PYZsh{} longform\PYZus{}dur\PYZus{}stats\PYZus{}3\PYZus{}mon\PYZus{}by\PYZus{}day}
\end{Verbatim}
\end{tcolorbox}

    \subparagraph{Put data from above into it's own DF for
swarmplot}\label{put-data-from-above-into-its-own-df-for-swarmplot}

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{170}{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
\PY{n}{longform\PYZus{}dur\PYZus{}stats\PYZus{}3\PYZus{}mon\PYZus{}by\PYZus{}day} \PY{o}{=} \PY{n}{pd}\PY{o}{.}\PY{n}{merge}\PY{p}{(}\PY{n}{longform\PYZus{}dur\PYZus{}stats\PYZus{}3\PYZus{}mon\PYZus{}by\PYZus{}day}\PY{p}{,} \PY{n}{hist\PYZus{}3\PYZus{}mon\PYZus{}day\PYZus{}index}\PY{p}{,} \PY{n}{how}\PY{o}{=}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{left}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{n}{on}\PY{o}{=}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Day}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}
\end{Verbatim}
\end{tcolorbox}
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
\PY{n}{longform\PYZus{}dur\PYZus{}stats\PYZus{}3\PYZus{}mon\PYZus{}by\PYZus{}day}\PY{p}{[}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Day\PYZus{}Label}\PY{l+s+s1}{\PYZsq{}}\PY{p}{]} \PY{o}{=} \PY{n}{longform\PYZus{}dur\PYZus{}stats\PYZus{}3\PYZus{}mon\PYZus{}by\PYZus{}day}\PY{p}{[}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{StartTimeStamp}\PY{l+s+s1}{\PYZsq{}}\PY{p}{]}\PY{o}{.}\PY{n}{astype}\PY{p}{(}\PY{n+nb}{str}\PY{p}{)} \PY{o}{+} \PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{ }\PY{l+s+se}{\PYZbs{}n}\PY{l+s+s1}{\PYZsq{}} \PY{o}{+} \PY{n}{longform\PYZus{}dur\PYZus{}stats\PYZus{}3\PYZus{}mon\PYZus{}by\PYZus{}day}\PY{p}{[}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Weekday}\PY{l+s+s1}{\PYZsq{}}\PY{p}{]}\PY{o}{.}\PY{n}{astype}\PY{p}{(}\PY{n+nb}{str}\PY{p}{)}
\PY{n}{longform\PYZus{}dur\PYZus{}stats\PYZus{}3\PYZus{}mon\PYZus{}by\PYZus{}day}\PY{o}{.}\PY{n}{set\PYZus{}index}\PY{p}{(}\PY{n}{keys}\PY{o}{=}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Day}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{n}{inplace}\PY{o}{=}\PY{k+kc}{True}\PY{p}{)}
\PY{n}{longform\PYZus{}dur\PYZus{}stats\PYZus{}3\PYZus{}mon\PYZus{}by\PYZus{}day}\PY{o}{.}\PY{n}{drop}\PY{p}{(}\PY{n}{axis}\PY{o}{=}\PY{l+m+mi}{1}\PY{p}{,} \PY{n}{columns}\PY{o}{=}\PY{p}{[}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{StartTimeStamp}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Weekday}\PY{l+s+s1}{\PYZsq{}}\PY{p}{]}\PY{p}{,} \PY{n}{inplace}\PY{o}{=}\PY{k+kc}{True}\PY{p}{)}
\end{Verbatim}
\end{tcolorbox}
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
\PY{n}{longform\PYZus{}dur\PYZus{}stats\PYZus{}3\PYZus{}mon\PYZus{}by\PYZus{}day}\PY{o}{.}\PY{n}{head}\PY{p}{(}\PY{p}{)}
\end{Verbatim}
\end{tcolorbox}

    \subparagraph{Swarmplot of stats by
day}\label{swarmplot-of-stats-by-day}
...
\PY{n}{colors} \PY{o}{=} \PY{n}{sns}\PY{o}{.}\PY{n}{color\PYZus{}palette}\PY{p}{(}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{\PYZsh{}89D328}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{\PYZsh{}D30702}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{\PYZsh{}1299E2}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{\PYZsh{}1F2C28}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}\PY{p}{)}  \PY{c+c1}{\PYZsh{} MIN, MAX, MEAN, COUNT}

\PY{n}{sns}\PY{o}{.}\PY{n}{swarmplot}\PY{p}{(}\PY{n}{x}\PY{o}{=}\PY{n}{longform\PYZus{}dur\PYZus{}stats\PYZus{}3\PYZus{}mon\PYZus{}by\PYZus{}day}\PY{o}{.}\PY{n}{index}\PY{p}{,} \PY{n}{y}\PY{o}{=}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Dur\PYZus{}Stat\PYZus{}Value}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{n}{hue}\PY{o}{=}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Dur\PYZus{}Stat\PYZus{}Type}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{n}{data}\PY{o}{=}\PY{n}{longform\PYZus{}dur\PYZus{}stats\PYZus{}3\PYZus{}mon\PYZus{}by\PYZus{}day}\PY{p}{,} \PY{n}{dodge}\PY{o}{=}\PY{k+kc}{True}\PY{p}{,} \PY{n}{edgecolor}\PY{o}{=}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{gray}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{n}{linewidth}\PY{o}{=}\PY{o}{.}\PY{l+m+mi}{4}\PY{p}{,} \PY{n}{palette}\PY{o}{=}\PY{n}{colors}\PY{p}{,} \PY{n}{s}\PY{o}{=}\PY{l+m+mi}{7}\PY{p}{,} \PY{n}{ax}\PY{o}{=}\PY{n}{ax}\PY{p}{)}
\PY{c+c1}{\PYZsh{} sns.regplot(x=\PYZsq{}Day\PYZsq{}, y=\PYZsq{}Mean\PYZus{}Duration\PYZsq{}, data=dur\PYZus{}stats\PYZus{}3\PYZus{}mon\PYZus{}by\PYZus{}day, color=\PYZsq{}\PYZsh{}1299E2\PYZsq{}, scatter\PYZus{}kws=\PYZob{}\PYZsq{}s\PYZsq{}: 0\PYZcb{}, ax=ax)}

\PY{n}{ax}\PY{o}{.}\PY{n}{set\PYZus{}yscale}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{symlog}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}  \PY{c+c1}{\PYZsh{} Retain zero values}
\PY{n}{ax}\PY{o}{.}\PY{n}{set\PYZus{}ylim}\PY{p}{(}\PY{n}{bottom}\PY{o}{=}\PY{o}{\PYZhy{}}\PY{l+m+mf}{0.25}\PY{p}{)}  \PY{c+c1}{\PYZsh{} 0 doesn\PYZsq{}t work... }

\PY{n}{ax}\PY{o}{.}\PY{n}{set\PYZus{}ylabel}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Request Duration Stats}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{n}{fontsize}\PY{o}{=}\PY{l+m+mi}{20}\PY{p}{)}
\PY{n}{ax}\PY{o}{.}\PY{n}{set\PYZus{}xlabel}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Day}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{n}{fontsize}\PY{o}{=}\PY{l+m+mi}{20}\PY{p}{)}
\PY{n}{ax}\PY{o}{.}\PY{n}{set\PYZus{}title}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Min, Max, Mean, and Count of Request Durations by Week (3 mo.)}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{n}{loc}\PY{o}{=}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{center}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{n}{fontsize}\PY{o}{=}\PY{l+m+mi}{25}\PY{p}{)}

\PY{n}{ax}\PY{o}{.}\PY{n}{tick\PYZus{}params}\PY{p}{(}\PY{n}{axis}\PY{o}{=}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{both}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{n}{which}\PY{o}{=}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{major}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{n}{labelsize}\PY{o}{=}\PY{l+m+mi}{15}\PY{p}{)}
\PY{n}{ax}\PY{o}{.}\PY{n}{set\PYZus{}xticklabels}\PY{p}{(}\PY{n}{longform\PYZus{}dur\PYZus{}stats\PYZus{}3\PYZus{}mon\PYZus{}by\PYZus{}day}\PY{p}{[}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Day\PYZus{}Label}\PY{l+s+s1}{\PYZsq{}}\PY{p}{]}\PY{p}{)}

\PY{n}{ax}\PY{o}{.}\PY{n}{legend}\PY{p}{(}\PY{n}{borderpad}\PY{o}{=}\PY{l+m+mi}{1}\PY{p}{,} \PY{n}{labelspacing}\PY{o}{=}\PY{l+m+mi}{1}\PY{p}{,} \PY{n}{fontsize}\PY{o}{=}\PY{l+m+mi}{18}\PY{p}{)}
\end{Verbatim}
\end{tcolorbox}
...
\begin{Verbatim}[commandchars=\\\{\}]
\PY{c+c1}{\PYZsh{} Add a function that pulls the data which allows the user to filter data IF WANTED (make a}
\PY{c+c1}{\PYZsh{} default that pulls all historical data?).}
\end{Verbatim}
\end{tcolorbox}
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{datacell_request_time_v2_files/datacell_request_time_v2_162_1.png}
    \end{center}
    { \hspace*{\fill} \\}
...
month}\label{total-health-history-stats-by-month}
    ...
    This section is under construction. My goal is to aggregate all the
historical data by month in SQL and then make a figure with a
subplot/Axes for each year and show all historical stats per month over
the whole history of the DB.

I just have to separate year and month from being in the same column in
the DF...
...
\end{tcolorbox}
    ...
    Below is code that does something similar, dut in pandas. It will
probably not be used. Running this currently may cause memory errors.
...
data}\label{plot-of-all-historical-stats-aggregated-monthly-with-subplotsaxes-for-each-year-all-data}
    ...
    This code will be used with the data loaded directly from SQL above,
once bugs are fixed...
...
week}\label{total-health-history-stats-by-week}
    ...
    The following code was also done using all historical data in a pandas
DF, so it will not be used as it takes a ton of memory.
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{datacell_request_time_v2_files/datacell_request_time_v2_187_0.png}
    \end{center}
    { \hspace*{\fill} \\}
